...
namespace ATMTECH.ShoppingCart.DAO
{
    public class DAOProduct : BaseDao<Product, int>, IDAOProduct
    {
        ...
        public IList<Product> GetProductsWithoutStock(int id)
        {
            return
                GetBySql(
            "select Product.Id,Product.Description,Product.IsActive,Product.DateCreated,Product.DateModified,Product.Language,Product.OrderId,Product.Search,Product.ComboboxDescription,Product.Ident,Product.UnitPrice,Product.CostPrice,Product.Enterprise,Product.Weight,Product.Supplier,Product.InternalIdent,Product.DescriptionFrench,Product.DescriptionEnglish,Product.ProductCategoryFrench,Product.ProductCategoryEnglish,Product.NameFrench,Product.NameEnglish,Product.IsNotOrderable, Product.SalePrice, Product.Brand, Product.IsSlideShow, UserLoginModified from Product Where Id not in (SELECT Product From Stock) and Enterprise = " + id);
        }
