...
namespace ATMTECH.Web
{
    ...
    public class CaptchaImage
    {
        ...
        private void GenerateImage()
        {
            ...
            int emSize = this.width * 2 / text.Length;
            FontFamily family = fonts[RandomNumberStrong.Next(fonts.Length - 1)];
            ...
            int red = RandomNumberStrong.Next(255), green = RandomNumberStrong.Next(255), blue = RandomNumberStrong.Next(255);
            while (red >= bcR && red - 20 <= bcR ||
                red < bcR && red + 20 >= bcR)
            {
                red = RandomNumberStrong.Next(0, 255);
            }
            SolidBrush sBrush = new SolidBrush(Color.FromArgb(red, green, blue));
            g.FillPath(sBrush, path);

            // Iterate over every pixel
            double distort = RandomNumberStrong.Next(5, 20) * (RandomNumberStrong.Next(10) == 1 ? 1 : -1);
