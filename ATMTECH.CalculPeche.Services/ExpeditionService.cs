...
namespace ATMTECH.CalculPeche.Services
{
    public class ExpeditionService : BaseService, IExpeditionService
    {
        ...
        public IList<Repartition> ObtenirRepartition(int idExpedition)
        {
            ...
            int nombreTotalJour = obtenirParticipantPresenceExpedition.Select(x => x.Date).Distinct().Count();
            int nombreTotalRepas = obtenirParticipantRepasExpedition.Sum(x => x.NombreRepas);

            List<DateTime> dateTimes = obtenirParticipantBateauExpedition.Select(x => x.Date).OrderBy(x => x.Date).Distinct().ToList();
            int nombreTotalSortieBateau = 0;
            int nombreTotalAutomobile = 0;
            foreach (DateTime dateTime in dateTimes)
            {
                if (obtenirParticipantBateauExpedition.Count(x => x.Date == dateTime && x.EstBateau) > 0)
                {
                    nombreTotalSortieBateau += 1;
                }

                if (obtenirParticipantAutomobileExpedition.Count(x => x.Date == dateTime && x.EstAutomobile) > 0)
                {
